...
def print_func():
    ...
    for elem in i2cList:                                        # Go through all connected I2C devices
        print("Channel " + str(elem['channel']) + ' type: ' + elem['device'] +', values: ' + str(elem['vals']))
    #
...
def adcScan():
    ...
    for el1 in adc.connectedDevices:                            # Check for connected and new devices
        if (len(filter(lambda el2: el2['device'] == el1['device'], adcListPrevious)) > 0):
    ...
    for el1 in adcListPrevious:                                 # Check for disconnected devices
        if (len(filter(lambda el2: el2['device'] == el1['device'], adcList)) == 0):
...
def i2cScan():
    ...
    for el1 in i2c.connectedDevices:                            # Check for connected and new devices
        if (len(filter(lambda el2: el2['device'] == el1['device'], i2cListPrevious)) > 0):
    ...
    for el1 in i2cListPrevious:                                 # Check for disconnected devices
        if (len(filter(lambda el2: el2['device'] == el1['device'], i2cList)) == 0):
