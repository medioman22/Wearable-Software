...
"""
#import RoboMUX as mux                                              # MUX class.
import I2C_BNO055                                                   # Driver for the BNO055 device
#import I2C_MPU6050                                                 # Driver for the MPU6050 device

class RoboI2C:
    """I2C device handling including MUX."""
    ...
    # Each channel will have a list of driver objects. These objects will be used to detect & use devices
    _scan_driver_objects = None

    # List of connected devices dictionary. Contains: {chn, subchn, device, dir, dim, vals, drv_idx}. Subchannel is -1 in case no MUX is connected.
    connected_devices = []

    # List of all 'devices' dictionary. Contains: {chn, device, dir, dim, vals, actv, sublist, drv_idx}
    # Sublist is a list of dictionaries for all MUXed values: {subchn, device, dir, dim, vals, actv, drv_idx}
    all_devices = []
    ...
    def __init__(self):
        ...
        for idx, chn in enumerate(self.SCAN_CHNS):
            ...
            self._scan_driver_objects.append(driver_obj_list)       # Add the object list to the channel collection

                                                                    # Create the entry for this channel dictionary
            self.all_devices.append({   'chn':          chn,
                                        'device':       '',
                                        'dir':          None,
                                        'dim':          0,
                                        'vals':         [],
    ...
    def _update_channel(self, chn_dict, subchn_dict):
        """
        Update the device with a given channel dictionary and subchannel dictionary.

            If subchannel is None, then we don't have any MUX attached.


            (Returns a new list of dictionaries on the form of {'chn', 'subchn', 'event':'disc'/'conn'/'none', 'device':device/'', 'mux':'disc'/'conn'/'none'})
        """
        ret_list = []
        ...
        if subchn_dict is not None:                                 # See if we have a MUX
            ...
            #         subchn_dict['vals'] = self._mux_object.func_on_mux_chn(chn_dict['chn'], subchn_dict['subchn'], drv_obj.getValues)
            ...
            #             subchn_dict['vals'] = self._mux_object.func_on_mux_chn(chn_dict['chn'], subchn_dict['subchn'], drv.getValues)
            ...
            #             subchn_dict['dir'] = self._mux_object.func_on_mux_chn(chn_dict['chn'], subchn_dict['subchn'], drv.getDir)
            #             subchn_dict['dim'] = self._mux_object.func_on_mux_chn(chn_dict['chn'], subchn_dict['subchn'], drv.getDim)
            #             chn_dict['actv'] = True
            ...
            if chn_dict['actv']:                                    # Check if device is connected
                ...
                if drv_obj.getDeviceConnected():
                                                                    # If still connected, get the updated values and save them
                    chn_dict['vals'] = drv_obj.getValues()
                else:                                               # If no longer connected -> reflect change
                    chn_dict['actv'] = False
        ...
        for elem in self.all_devices:
            if elem['actv']:
                if len(elem['sublist']) > 0:
                    ...
                    self.connected_devices.append({ 'chn':      elem['chn'],
                                                    'subchn'    :-1,
                                                    'device'    :elem['device'],
                                                    'dir'       :elem['dir'],
                                                    'dim'       :elem['dim'],
                                                    'vals'      :elem['vals'],
                                                    'drv_idx'   :elem['drv_idx']})
        return ret_list # TODO: Remove return of update function
