...
class Input:
    ...
    def _validatePin(self, pin):
        ...
        for pinConfig in PIN_MAP["INPUT"]:                      # Loop all available pin configs
            if (pinConfig['DATA'] == pin):                      # Look for pin
                return True                                     # Return True if pin listed
        else:
            return False                                        # Return False for no pin

    def detectPin(self, pin):
        ...
        for device in self.connectedDevices:                    # Loop all connected devices
            for drv in self._connectedDrivers:                  # Loop all connected drivers
                if (device['mux'] == -1):                       # Unmuxed pin
                    if device['device'] == '{}@Input[{}]'.format(drv.getDevice(), drv.getPin()): # Match for drv and device
                        device['vals'] = drv.getValues()        # Update values
                else:
                    #print(device['device'],'{}@Input[{}:{}]'.format(drv.getDevice(), drv.getPin(), drv.getMuxedPin()))
                    if device['device'] == '{}@Input[{}:{}]'.format(drv.getDevice(), drv.getPin(), drv.getMuxedPin()): # Match for drv and device
    ...
    def scan(self):
        ...
        for pinConfig in PIN_MAP["INPUT"]:                      # Loop all available pin configs
            ...
            #print('PIN DETECT', self.detectPin(muxSwitch), 'MUX DETECT', MuxShadow.detect())
            if not self.detectPin(muxSwitch) or not MuxShadow.detect(): # Not a muxed pin or mux is not enabled
                for lastDrv in lastConnectedDrivers:            # Test last connected drivers
                    ...
                    for possibleDrv in DRIVERS:                 # Test all drivers
                        drv = InputBasic(pin)                   # Test the different drivers
                ...
                for muxedPin in range(Mux.range):               # Loop all muxed pins
                    ...
                    for lastDrv in lastConnectedDrivers:        # Test last connected drivers
                        ...
                        for possibleDrv in DRIVERS:             # Test all drivers
                            drv = InputBasic(pin, muxedPin)     # Test the different drivers
                            if not drv.getDeviceConnected():    # Validate driver connected
                                continue                        # Try next driver until none is left
                            drv.configureDevice()               # Configure device
                            self._connectedDrivers.append(drv)  # Add to connected driver list
