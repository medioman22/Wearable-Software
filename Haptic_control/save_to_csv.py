...
def experiment(experimentType = 'intensity'):
    global nbOfSet
    print('First, you will experiment each ', experimentType, ' and a feedback will be given to you. Press space to activate each direction')
...
def random_set(nbOfSet = 1, exp = 'direction', feedbackRequest = True, save = True):
    global initialDirectionList, initialIntensityList, maxLength, maxIntensity, dictOfCorresp, subject
    valueList = []
    correctList = []
    givenList = []
    timeList = []
    ...
    for i in range(0, nbOfSet):
        ...
        elif exp == 'intensity' : valueList.extend(initialIntensityList)
        else :
    ...
    for i in range(0, len(valueList)):
        ...
        elif exp == 'intensity' : correctList = valueList
    if exp == 'intensity' : valueList = [i*maxIntensity/5 for i in valueList]
    ...
    print(timeList)
    if save : 
        with open(subject + '_' + exp + '_intens_feedback.csv', 'w') as csvfile:
